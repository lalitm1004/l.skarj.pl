<script lang="ts">
    ...
    onMount(() => {
        const updateWidget = async() => {
            const updateResponse = await fetch("/api/getWidgetDetails");
...
<div class={`lastfm absolute top-2 right-2 bg-spotify-black flex justify-start items-center h-auto min-h-[90px] w-auto min-w-[250px] rounded-md`}>
    ...
    {/if}
    <div class={`flex justify-center ml-1 mr-4 py-2 flex-grow h-full text-white font-terminal`}>
        <div class={`flex flex-col justify-center`}>
            {#if loading}
                <p>loading...</p>
            {:else}
                <p class={`text-spotify-gray text-sm`}>{playingNow ? "Currently playing" : "Last played"}</p>
                <p>{songName}</p>
                <p>{artistName}</p>
            {/if}
        </div>
        {#if previewUrl}
            <div class={`flex justify-center items-center`}>
                <button on:click={handleClick}>
                    <img
                        class={`w-[75px] h-[75px]`}
                        src={`assets/${playingPreview ? "pause" : "play"}.svg`}
                        alt={`${playingPreview ? "pause" : "play"} preview`}
                    />
